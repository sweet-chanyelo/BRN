...
if __name__ == '__main__':
    ...
    for i in range(len(labelset)):
        labelset[i] = int(labelset[i] * 100)
    test_dataset = input[lamda]    # 测试集
    test_labelset = output[lamda]  # 测试集标签
    for i in range(len(test_labelset)):
        test_labelset[i] = int(test_labelset[i] * 100)
    # 标准化
    ...
    HMM = hmm.GaussianHMM()
    # 训练集
    HMM.fit(x_train_Standard, labelset)

    # 测试集
    test_out = HMM.predict(x_test_Standard)
    # 训练集
    train_out = HMM.predict(x_train_Standard)
    # 输出
