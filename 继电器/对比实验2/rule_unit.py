...
def integration(t, H):
    ...
    for i in range(0, 11):
        input1[25 + 4 * i: 25 + 4 * i + 4] = H[0][125 + 5 * i + 1: 125 + 5 * i + 5]  # 后58个数据的80%
        input2[25 + 4 * i: 25 + 4 * i + 4] = H[1][125 + 5 * i + 1: 125 + 5 * i + 5]  # 后58个数据的80%
        output1[25 + 4 * i: 25 + 4 * i + 4] = output[125 + 5 * i + 1: 125 + 5 * i + 5]  # 后58个数据的80%
    # print('@@@@@@', len(input1), len(input2), len(output1))
    ...
    for train_index, test_index in KF.split(H[0], H[1], output):
        ...
        # 产生规则单元权重
        omega = np.random.randn(2)  # (-1,1)之间
        # -----------------------------cmaes---------------------------#
        q = np.zeros(len(input1))
        FES = 0
        N = len(omega)  # 维数
        xmean = omega   # 初始值
        sigma = 0.5     # 步长(标准差)
    ...
    # 该训练集的最优参数取均值
        ...
        for j in range(0, len(input1)):
            q[j], blank = error(P[point], input1[j], input2[j], output1[j])  # 调用目标函数
        err.append(sum(q) / len(input1))
        # print("%%%%%%", err)
    # print('第{}个规则单元测试误差为：{}统计参数值为：{}'.format(t, err[point], P[point]))
        point += 1
    # 交叉验证后的统计泛化误差
    err_index = np.argsort(err)
    print('第{}个规则单元最优误差为：{}，结构参数为：{}'.format(t, err[err_index[0]], P[err_index[0]]))
    Para = P[err_index[0]]  # 取得最优泛化误差的作为最优结构参数
