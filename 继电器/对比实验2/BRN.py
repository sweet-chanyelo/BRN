...
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&外准则筛选&&&&&&&&&&&&&&&&&&&&&&&&#
    ...
    output_ture = []
    for i in range(0, len(best_belief[0])):  # 数据量
    ...
    for i in range(0, 25):
        # 存放数据
        output2[i] = output_ture[5 * i]  # 前125个数据的20%
    for i in range(0, 11):
        output2[25 + 4 * i: 25 + 4 * i + 4] = output_ture[125 + 5 * i + 1: 125 + 5 * i + 5]  # 后58个数据的80%
    # output_excel = pd.DataFrame(output2)
