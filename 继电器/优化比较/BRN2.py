...
from unoptimize import unoptimize
...
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&外准则筛选&&&&&&&&&&&&&&&&&&&&&&&&#
    ...
    print(err_index)
    print('统计误差{}， 最优结构参数下所有数据的置信分布输出{}'.format(err, belief[err_index[0]]))
    ...
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%并行处理置信规则网络%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
        ...
        p2 = newPool(3)
        res2 = p2.map(unoptimize, range(len(ip2)), H)
        #  返回误差及置信度
        # print(res2)
        ...
        if err[err_index[0]] < best_err:
            ...
            best_belief = belief
        elif err[err_index[0]] >= best_err and layer > 10:
            break
        errr.append(err[err_index[0]])
        layer += 1
    # 结论等级参考值(由用户给定)
    ...
    print(errr)
    plt.plot(output_ture)
    # 画图
    output_excel = pd.DataFrame(output_ture)
    writer = pd.ExcelWriter('brn2.xls')
    output_excel.to_excel(writer, sheet_name='brn2', startcol=0, index=False)
    writer.save()
    plt.show()