...
def integration(t, H):
    ...
    input1 = np.zeros((69, 4))
    input2 = np.zeros((69, 4))
    output1 = np.zeros(69)
    for i in range(0, 25):
        # 存放数据
        input1[i] = H[0][5 * i]      # 前125个数据的20%
        input2[i] = H[1][5 * i]      # 前125个数据的20%
        output1[i] = output[5 * i]   # 前125个数据的20%
    for i in range(0, 11):
        input1[25 + 4 * i] = H[0][125 + 5 * i + 1]  # 后58个数据的80%
        input1[25 + 4 * i + 1] = H[0][125 + 5 * i + 2]
        input1[25 + 4 * i + 2] = H[0][125 + 5 * i + 3]
        input1[25 + 4 * i + 3] = H[0][125 + 5 * i + 4]
        input2[25 + 4 * i] = H[1][125 + 5 * i + 1]  # 后58个数据的80%
        input2[25 + 4 * i + 1] = H[1][125 + 5 * i + 2]
        input2[25 + 4 * i + 2] = H[1][125 + 5 * i + 3]
        input2[25 + 4 * i + 3] = H[1][125 + 5 * i + 4]
        output1[25 + 4 * i] = output[125 + 5 * i + 1]  # 后58个数据的80%
        output1[25 + 4 * i + 1] = output[125 + 5 * i + 2]
        output1[25 + 4 * i + 2] = output[125 + 5 * i + 3]
        output1[25 + 4 * i + 3] = output[125 + 5 * i + 4]
    for train_index, test_index in KF.split(input1, input2, output1):
        train_x1 = input1[train_index]   # 训练集索引
        train_x2 = input2[train_index]   # 训练集索引
        train_y = output1[train_index]  # 训练集标签索引
        test_x1 = input1[test_index]   # 测试集
        test_x2 = input2[test_index]   # 测试集
        test_y = output1[test_index]  # 测试集标签
        # 产生规则单元权重
        ...
        while counteval < stopeval:  # 生成并评估所有后代, 第一代
            for k in range(0, lamda):  # 第一个后代
                ...
                for j in range(0, len(input1)):
                    temp[j], blank = error(arx[:, k], input1[j], input2[j], output1[j])  # 调用目标函数
                arfitness[k] = np.sum(temp) / len(input1)
    ...
    # 该训练集的最优参数取均值
        ...
        # 测试集
        for j in range(0, len(input1)):
            q, blank = error(P[point], input1[j], input2[j], output1[j])  # 调用目标函数
            a.append(q)
