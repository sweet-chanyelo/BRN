...
# **************************初始信息********************************#
    ...
    theta = p[0:9]            # 规则权重
    delta = p[9:11]            # 属性权重
    belta = np.array(p[11:]).reshape((-1, 3))    # 置信度
...
# &&&&&&&&&&&&&&&&&&&&&&&&&&&&规则的融合&&&&&&&&&&&&&&&&&&&&&&&#
        ...
        for k in range(sum_l):
            m[k] = omega[k] * belta[k]
        belief[l] = agg(m, sum_l, sum_d, omega, belta, pointer)
        # print(belief)                           # 检验置信分布
# =================================外准则评估===========================#
    ...
    # print("误差为：%s" % (np.sum((ture_z - test_z) ** 2) / len(test_z)))
    # plt.figure(1)
    # plt.plot(ture_z, 'g', lw=1)
    # # plt.ylim(-1, 10)
    # plt.figure(2)
    # plt.plot(ture_z_2, 'b', lw=1)
    # plt.ylim(0, 100)