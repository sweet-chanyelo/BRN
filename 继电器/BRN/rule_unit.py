...
def integration(t, H):
    ...
    # print(output)
    # 初始化
    sum_t = sum_l = len(H)          # 规则数=单元数sum_t
    ...
    for train_index, test_index in KF.split(H[0], H[1], output):
        ...
        for j in range(0, len(train_x1)):
                ...
                for k in range(0, lamda):  # 第一个后代
                    ...
                    for i in range(0, N):  # omega相对权重
                        if arx[i][k] > 1:
                            arx[i][k] = 1
                    # % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
                    arfitness[k], blank = error(arx[:, k], train_x1[j], train_x2[j], train_y[j])  # 调用目标函数
            ...
            p[t] = bestx
        # 该训练集的最优参数取均值
        P[point] = [sum(p[:, 0]) / max(sum(p[:, 0]), sum(p[:, 1])), sum(p[:, 1]) / max(sum(p[:, 0]), sum(p[:, 1]))]
        # 测试集
        for j in range(0, len(test_x1)):
            a[j], blank = error(P[point], test_x1[j], test_x2[j], test_y[j])  # 调用目标函数
        err[point] = sum(a) / len(a)
