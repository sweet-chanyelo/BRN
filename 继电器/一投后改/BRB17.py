...
def brb(p, input):
    ...
    feature1 = [0.2, 0.5, 1.21]
    feature2 = [0.38, 0.4, 0.43]
...
def demo_func(p):
    ...
    for i in range(1, row):
        input[i - 1][0] = df1.iat[i, 6]
        input[i - 1][1] = df1.iat[i, 7]
...
# -----------------运行-----------------------------------------------------------------------------------%#
    ...
    train_x = input[index]  # 训练集
    train_y = output[index]  # 训练集标签
    test_x = np.delete(input, index, 0)  # 测试集
    test_y = np.delete(output, index, 0)  # 测试集标签
    # index = range(0, len(output), 5)
    # train_x = np.delete(input, index, 0)  # 训练集
    # train_y = np.delete(output, index, 0)
    # test_x = input[index]  # 测试集
    ...
    writer = pd.ExcelWriter('BRB17.xlsx')
